...
class Spool:
    ...
    def __str__(self):
        return f"""
        Run {self.name} in {self.folder} with config {self.run_config} at {self.timestamp}
        """
...
class Theseus:
    ...
    def resume_or_start(self, name: str, notes: str, run_config: dict):
        """
        ...
        try:
            ...
            return self.start(name, notes, run_config)

    def start(self, name: str, notes: str, run_config: dict):
        """
...
def cli():
    ...
    show_parser.add_argument("field", type=str, help="Field to show (e.g., 'name', 'folder')", default="all")
    ...
    match args.command:
        case "list":
            ...
            for exp in experiments:
                print(exp[0])  # Print only the name of each experiment
        case "query":
        ...
        case "show":
            ...
            for exp in matches:
                for field in fields:
                    if args.field == "all" or field == args.field:
                        pp.pprint(f"{field}: {getattr(exp, field)}")
                print("-----------------")
        case _:
            parser.print_help()
            exit(1)
